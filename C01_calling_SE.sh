...
# qsub -l vf=5G -pe parallel 2 -N waVCPCsam $SAMEXE >>$PATH_LOGS/SGE_C01_${i}.txt
#run gatk
# echo "run gatk:"
...
# GATEXE="$PATH_SCRIPTS/run_gatk_SE.sh $REF_FA $BAM_MOD $PATH_GATK_DATA/$OUTDIR $PATH_GATK $PATH_PICARD $PATH_SAMTOOLS"
# qsub -l vf=5G -pe parallel 2 -N waVCPCgat $GATEXE >>$PATH_LOGS/SGE_C01_${i}.txt
#run varscan
echo "run varscan:"
...
qsub -l vf=5G -pe parallel 2 -N waVCPCvs2 $VS2EXE >>$PATH_LOGS/SGE_C01_${i}.txt
#run lofreq
# echo "run lofreq:"
...
# qsub -l vf=4G -pe parallel 2 -N waVCPClo1 $LO1EXE >>$PATH_LOGS/SGE_C01_${i}.txt
#run lofreq2
echo "run lofreq2:"
...
qsub -l vf=4G -pe parallel 2 -N waVCPClo2 $LO2EXE >>$PATH_LOGS/SGE_C01_${i}.txt
...
##run cortex
cd $CURRENT_DIR
COREXE="$PATH_SCRIPTS/run_cortex_grid_SE.sh $BAM_MOD $OUTDIR $i $BAM_BASE"
# qsub -l vf=6G -pe parallel 2 -N waVCPCcor $COREXE >>$PATH_LOGS/SGE_C01_${i}.txt
# bash $COREXE
