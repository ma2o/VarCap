...
for var in $( ls | grep -E 'true_pos.vcf'); do
  if (($ITER < $MAX_ITER)); then
    cat $var | grep -e 'SNP' | grep -e 'varscan'| cut -f1  >>varscan.all;
    cat $var | grep -e 'SNP' | grep -e 'lofreq'| grep -v 'lofreq2' | cut -f1 >>lofreq1.all;
    cat $var | grep -e 'SNP' | grep -e 'lofreq2'| cut -f1  >>lofreq2.all;
    cat $var | grep -E 'DEL|INS|IND|LI' | grep -Ev 'DUP|ITX' | grep -e 'varscan' | cut -f1 >>varscan_indel_small.all;
    cat $var | grep -E 'DEL|INS|IND|LI' | grep -Ev 'DUP|ITX' | grep -e 'pindel' | awk '{ if ($7 <= 10) print }' | cut -f1 >>pindel_indel_small.all;
    cat $var | grep -E 'DEL|INS|IND|LI' | grep -Ev 'DUP|ITX' | grep -e 'pindel' | awk '{ if ($7 > 10) print }' | cut -f1 >>pindel_indel_large.all;
    cat $var | grep -E 'DEL|INS|IND|LI' | grep -Ev 'DUP|ITX' | grep -e 'breakdancer' | cut -f1 >>breakdancer_indel_large.all;
    cat $var | grep -E 'DEL|INS|IND|LI' | grep -Ev 'DUP|ITX' | grep -e 'delly' | cut -f1 >>delly_indel_large.all;
    cat $var | grep -E 'DEL|INS|IND|LI|COMPLEX' | grep -Ev 'DUP|ITX' | grep -e 'cortex' | cut -f1 >>cortex_indel_large.all;
    
    cat $var | grep -E 'DUP|ITX' | grep -e 'pindel' | awk '{ if ($6 != "DEL") print }' | cut -f1 >>pindel_sv.all;
    cat $var | grep -E 'DUP|ITX' | grep -e 'pindel' | awk '{ if ($6 == "DEL") print }' | cut -f1 >>pindel_sv_del.all;
    cat $var | grep -E 'DUP|ITX' | grep -e 'breakdancer' | awk '{ if ($6 != "DEL") print }' | cut -f1 >>breakdancer_sv.all;
    cat $var | grep -E 'DUP|ITX' | grep -e 'breakdancer' | awk '{ if ($6 == "DEL") print }' | cut -f1 >>breakdancer_sv_del.all;
    cat $var | grep -E 'DUP|ITX' | grep -e 'delly' | awk '{ if ($6 != "DEL") print }' | cut -f1 >>delly_sv.all;
    cat $var | grep -E 'DUP|ITX' | grep -e 'delly' | awk '{ if ($6 == "DEL") print }' | cut -f1 >>delly_sv_del.all;
    cat $var | grep -E 'DUP|ITX' | grep -e 'cortex' | cut -f1 >>cortex_sv.all;
    
    cat $var | grep -E 'INV' | grep -e 'pindel' | cut -f1 >>pindel_inv.all;
    cat $var | grep -E 'INV' | grep -e 'delly' | cut -f1 >>delly_inv.all;
    cat $var | grep -E 'INV' | grep -e 'cortex' | cut -f1 >>cortex_inv.all;
...
echo "Total variant count over all iterations: $ITER of $COUNTER"
...
echo -e "Average detection rates per iteration $TOT_SNP $MAX_ITER $(cat varscan.all | wc -l)"
