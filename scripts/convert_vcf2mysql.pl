...
sub reformat_snpeff {
	...
	foreach my $snpeff_value (@snpeff_entries){
		...
		my ($snpeff_location,$snpeff_details) = split("\\(",$snpeff_value);
		...
		chop($snpeff_details);
		# print "snpeff_details".$snpeff_details."\n";
		my @snpeff_details_values = split("\\|",$snpeff_details);
		# print Dumper \@snpeff_details_values;
		if ( scalar @snpeff_gene_temp < "1"){
			@snpeff_gene_temp = ($snpeff_details_values[8]);
		} else {
			push(@snpeff_gene_temp,$snpeff_details_values[8]);
		}
	}
